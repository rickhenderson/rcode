...
rankall_old <- function(outcome, num = "best") {
     ## Read outcome data
     ...
     myordered<-lapply(splitdf, function(x) {x[order(x[,1]),]})
     
     output <- vector()
     
     for (i in length(states)) {
          statedata <- df[grep(states[i],data$state),]
          if (num == "worst") {
               num == nrow(statedata)
          }
          output <- append (output, as.character(statedata[num,1]))
